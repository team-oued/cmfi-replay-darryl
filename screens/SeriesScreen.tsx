    ...
    if (variant === 'list') {
        return (
            <div
                ...
                className={`flex items-center space-x-4 p-3 md:p-4 rounded-lg md:rounded-xl hover:bg-gray-100 dark:hover:bg-gray-800 transition-all duration-300 cursor-pointer group ${
                    serie.is_premium ? 'ring-2 ring-amber-400/40 bg-gradient-to-r from-amber-950/20 to-transparent' : ''
                }`}
            >
                <div
                    className={`relative w-20 h-20 md:w-24 md:h-24 bg-gray-300 dark:bg-gray-700 rounded-lg md:rounded-xl overflow-hidden flex-shrink-0 group-hover:scale-105 transition-transform duration-300 ${
                        serie.is_premium ? 'ring-2 ring-amber-400/60' : ''
                    }`}
                >
                    {serie.is_premium && (
                    ...
                    <img src={serie.imageUrl} alt={serie.title} className="w-full h-full object-cover" />
                    <div
                        onClick={handleSelect}
                    ...
                    >
                        <div className="flex items-center gap-2 px-4 py-2 bg-white/20 backdrop-blur-md rounded-lg border border-white/30">
                ...
                <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                        <h3 className="text-gray-900 dark:text-white font-bold text-base md:text-lg truncate">
                            {serie.title}
                        </h3>
                        {serie.is_premium && (
                            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-gradient-to-r from-amber-400 to-yellow-500 text-black flex-shrink-0">
                                <CrownIcon />
                                Premium
                            </span>
                        )}
                ...
                <button
                    className="p-2 rounded-full hover:bg-gray-200 dark:hover:bg-gray-700 z-10 transition-colors"
                    onClick={(e) => {
                        e.stopPropagation();
                        handleSelect();
                    }}
                ...
                >
                    <svg className="w-6 h-6 md:w-7 md:h-7 text-gray-800 dark:text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <div className="text-center py-20">
                        <div className="max-w-md mx-auto space-y-4">
                            ...
                            <h3 className="text-xl font-bold text-gray-900 dark:text-white">
                                {searchTerm ? t('noResults') || 'Aucun résultat' : t('noSeries') || 'Aucune série'}
                            </h3>
